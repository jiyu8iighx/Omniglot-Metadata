...
class PathCollector:
    ...
    def normalize_base_path(self, path: str) -> str:
        """标准化基础路径，去除前缀"""
        # 移除路径前缀
    ...
    def check_file_exists(self, base_path: str) -> bool:
        ...
        # 检查language目录
        lang_file = self.language_dir / normalized
        if lang_file.exists():
            return True
            
        # 检查writing目录  
        writing_file = self.writing_dir / normalized
    ...
    def collect_from_language_csv(self) -> List[Dict]:
        """从language.csv收集路径"""
        paths = []
        csv_file = self.base_dir / "language.csv"
    ...
    def collect_from_writing_csv(self) -> List[Dict]:
        """从writing.csv收集路径"""
        paths = []
        csv_file = self.base_dir / "writing.csv"
    ...
    def collect_from_langalph_single_csv(self) -> List[Dict]:
        ...
        paths = []
        csv_file = self.base_dir / "langalphSingle.csv"
    ...
    def collect_from_langalph_map_json(self) -> List[Dict]:
        ...
        paths = []
        json_file = self.base_dir / "langalphMap.json"
